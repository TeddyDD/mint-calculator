...
store Calculator.Store {
	...
	fun negate() : Void {
		next {
			data = { data | 
                current = Math.negate(data.current)
            }
        }
    }
	...
	fun setNumber(btn : String) : Void {
		next {
			data = { data |
				current = Maybe.withDefault(0, Number.fromString(resultString))
			}
		}
	} where {
		currentString = Number.toString(data.current)
		resultString = String.concat([currentString, btn])
    }
	...
	fun percent() : Void {
		...
		result = data.acc * data.current / 100.0
    }
