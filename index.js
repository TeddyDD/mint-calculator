...
class $$CalcData extends Record {}

$$CalcData.mappings = {"acc":null,"current":null,"operator":null,"equals":null}

$$CalcData.decode = (_input) => {
  let acc = Decoder.field(`acc`, Decoder.number)(_input)
  if (acc instanceof Err) { return acc }

  let current = Decoder.field(`current`, Decoder.number)(_input)
  if (current instanceof Err) { return current }

  let operator = Decoder.field(`operator`, Decoder.string)(_input)
  if (operator instanceof Err) { return operator }

  let equals = Decoder.field(`equals`, Decoder.boolean)(_input)
  if (equals instanceof Err) { return equals }

  return new Ok(new $$CalcData({
    acc: acc.value,
...
const $Calculator_Store = new (class extends Store {
    constructor() {
    super()
    this.state = {
        data: new $$CalcData({
  ...
  negate() {
    return new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    data: _update(this.data, { current: $Math.negate(this.data.current) })
    })), _resolve)
    })
  }
  ...
  doOp(acc, operator, current) {
    return (() => {
      let __condition = operator

       if (_compare(__condition, `+`)) {
        return acc + current
      } else if (_compare(__condition, `-`)) {
        return acc - current
      } else if (_compare(__condition, `*`)) {
        return acc * current
      } else if (_compare(__condition, `/`)) {
  ...
  percent() {
    let result = this.data.acc * this.data.current / 100.0

    return new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    data: _update(this.data, { current: result, equals: false })
    })), _resolve)
    })
  }

  equals() {
    let backup = this.data.current

    return (_compare(this.data.equals, true) ? new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    data: _update(this.data, { current: $Calculator_Store.doOp.bind($Calculator_Store)(this.data.acc, this.data.operator, this.data.current), equals: true })
    })), _resolve)
    }) : new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    data: _update(this.data, { current: $Calculator_Store.doOp.bind($Calculator_Store)(this.data.acc, this.data.operator, this.data.current), acc: backup, equals: true })
    })), _resolve)
    }))
  }
  ...
  mapButtonToOp(btn) {
    return (() => {
      let __condition = btn
       ...
       if (_compare(__condition, `C`)) {
        return $Calculator_Store.clear.bind($Calculator_Store)()
      } else if (_compare(__condition, `+/-`)) {
        return $Calculator_Store.negate.bind($Calculator_Store)()
      } else if (_compare(__condition, `+`)) {
        return $Calculator_Store.setOperator.bind($Calculator_Store)(`+`)
      } else if (_compare(__condition, `-`)) {
        return $Calculator_Store.setOperator.bind($Calculator_Store)(`-`)
      } else if (_compare(__condition, `×`)) {
        return $Calculator_Store.setOperator.bind($Calculator_Store)(`*`)
        ...
        return $Calculator_Store.percent.bind($Calculator_Store)()
      } else if (_compare(__condition, `=`)) {
        return $Calculator_Store.equals.bind($Calculator_Store)()
      } else {
        return $Calculator_Store.setNumber.bind($Calculator_Store)(btn)
      }
    })()
  }
})
...
class $Main extends Component {
  render() {
    return _createElement("div", {
      className: `main-base`
    }, [_createElement("h1", {}, [`Mint Calculator`]), _createElement($Calculator, {  }), _createElement($DownloadButton, { "href": `https://github.com/TeddyDD/mint-calculator` }, _array(`Get source code`))])
  }
}
...
class $DownloadButton extends Component {
  ...
  render() {
    return _createElement("a", {
      "href": this.href,
      "target": `_blank`,
      className: `download-button-base`
    }, [this.children])
  }
}
...
class $Calculator extends Component {
  ...
  get current () {
    if (this.props.current != undefined) {
      ...
      return $Number.toString(this.data.current) + this.debugString
    }
  }

  get data () { return $Calculator_Store.data }

  clear (...params) { return $Calculator_Store.clear(...params) }

  mapButtonToOp (...params) { return $Calculator_Store.mapButtonToOp(...params) }

  componentWillUnmount () {
    $Calculator_Store._unsubscribe(this)
  }

  componentDidMount () {
    $Calculator_Store._subscribe(this)
  }

  getClickCallback(btn) {
    return ((event) => {
    return this.mapButtonToOp.bind(this)(btn)
    })
  }
...
_insertStyles(`
  ...
  .download-button-base hover {
    background: linear-gradient(to bottom, #a5cc52 5%, #b8e356 100%);
    filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#a5cc52', endColorstr='#b8e356',GradientType=0);
    background-color: #a5cc52;
  }

  .download-button-base active {
    position: relative;
    top: 1px;
  }
